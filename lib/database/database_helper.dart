...
class DatabaseHelper {
  ...
  void _onCreate(Database db, int version) async {
    ...
    await db.execute(
        "ALTER TABLE Expenditure ADD COLUMN categorytype INTEGER DEFAULT 0");
    await db.execute(
        "CREATE TABLE Category(id INTEGER PRIMARY KEY, categoryname TEXT)");
    await db.execute(
        "ALTER TABLE Category ADD COLUMN categorytype INTEGER DEFAULT 0");
  }
  ...
  Future<List<Category>> getCategories(int swapIndex) async {
    var dbClient = await db;
    var query = "SELECT * FROM Category WHERE categorytype = " +
        swapIndex.toString() +
        " ORDER BY id DESC";
    ...
    for (int i = 0; i < list.length; i++) {
      var category =
          new Category(list[i]["categoryname"], list[i]["categorytype"]);
  ...
  Future<List<Expenditure>> getExpenses(
      int month, int year, int mode, int type,int day) async {
    var dbClient = await db;
    var query;
    if (mode == 0) {
      ...
      query = "SELECT * FROM Expenditure WHERE CAST(strftime('%m', strftime('%s',date(entrydate)), 'unixepoch') AS INTEGER)  = " +
          ...
          " AND  CAST(strftime('%Y', strftime('%s',date(entrydate)), 'unixepoch') AS INTEGER)  = " +
          year.toString() +
          " AND  CAST(strftime('%d', strftime('%s',date(entrydate)), 'unixepoch') AS INTEGER)  = " +
          day.toString() +
          " AND categorytype = " +
          type.toString() +
          " ORDER BY entrydate DESC";
  ...
  Future<int> deleteCategory(int id) async {
    var dbClient = await db;
    int res =
        await dbClient.rawDelete('DELETE FROM Category WHERE id = ?', [id]);
    return res;
  }
